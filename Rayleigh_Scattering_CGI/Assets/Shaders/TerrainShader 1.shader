Shader "Custom/EarthTerrainShader" {
    Properties {
    ...
    SubShader {
        ...
        void surf (Input IN, inout SurfaceOutput o) {
            ...
            float3 grassColor1 = float3(0.8, 0.8, 0.0);
            float3 grassColor2 = float3(0.5, 1.0, 0.0);
            float3 grassColor3 = float3(0.2, 0.6, 0.0);
            float3 stoneColor = float3(1, 1, 1);
            float3 soilColor = float3(0.4, 0.3, 0.05);
            float3 rockColor1 = float3(0.3, 0.0, 0.3);
            float3 rockColor2 = float3(1.0, 0.0, 0.0);
            float3 rockColor3 = float3(0.7, 0.5, 0.5);

            // Calculates the steepness and height
            float steepness = 1.1 - IN.worldNormal.y;
            float height = IN.worldPos.y;

            // Uses noise to add variety to the grass color
            float3 grassColor = lerp(lerp(grassColor1, grassColor2, noise(IN.worldPos.xz * 1.9)), grassColor3, noise(IN.worldPos.xz * 2.7));

            // Uses noise and steepness to blend between rock colors
            float3 rockColor = lerp(lerp(rockColor1, rockColor2, noise(IN.worldPos.xz * 2.1)), rockColor3, noise(IN.worldPos.xz * 2.3));

            // Defines color weights based on steepness and height
            ...
            float rockWeight = max(0, smoothstep(0.5, 1.0, steepness));
            ...
            // Computes final color as a weighted sum of terrain colors
